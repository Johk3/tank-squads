return function(ctx)
  ...
  test('escort plumbing: membership listeners run in order and patrol still reacts', function()
    ...
    patrol.add_waypoint(1, 3, {x = 0, y = 0}, a.surface)
    patrol.add_waypoint(1, 3, {x = 40, y = 0}, a.surface)
    patrol.start(1, 3)
    ...
    assert(a.command and a.command.destination.x == 20, 'patrol listener no longer resends')
  end)
  ...
  test('escort gui: division caption shows ward, formation and waiting', function()
    ...
    local caption = game.players[1].gui.screen.tank_squads_divisions.body.divisions.row_3.division_3.caption
