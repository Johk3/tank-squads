...
local r = storage.divisions[pindex].slots[1].patrol
...
if not storage.divisions[pindex].slots[1].patrol then error("turning patrol mode off should leave the route running until a new order") end
...
if storage.divisions[pindex].slots[1].patrol then error("explicit move did not replace the old patrol route") end
a.destroy()
