...
local ok, result = pcall(function()
  ...
  local soldiers = {}
  for i = 1, 11 do
    local e = s.create_entity{name = ({'tank-squad-flame','tank-squad-siege','tank-squad-soldier-1'})[(i-1)%3+1], position = {2990 + i * 2, 3010}, force = 'player'}
    created[#created + 1] = e
    soldiers[i] = e
  end
  ...
  for _ = 1, 120 do remote.call('tank-squads', 'tick') end
  if hurt.health < hurt.max_health * 0.95 then error('barracks did not heal the soldier: ' .. hurt.health) end
  remote.call('tank-squads', 'escort_tick')
...
return 'PASS: mixed specialists escort; heavy flame tank retreats, heals and rejoins with guard'
